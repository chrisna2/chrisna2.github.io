...
.bat 스크립트를 사용하여 SQL*Plus에 접속한 후, 하나의 디렉토리에 있는 모든 .sql 파일을 실행하고 결과를 spool을 사용하여 출력하려면 다음과 같이 스크립트를 작성할 수 있습니다.

먼저 모든 .sql 파일을 실행하기 위해 run_all_sql.sql 스크립트를 생성합니다.
...
column script new_value script_name
select null script from dual;

prompt Running all SQL scripts in &directory...

begin
  for f in (select filename from table(dirlist('&directory.', 'sql'))) loop
    execute immediate 'set termout on';
    execute immediate 'spool ' || '&directory.' || f.filename || '.log';
    execute immediate '@' || '&directory.' || f.filename;
...
이 스크립트는 디렉토리에서 .sql 파일을 찾고 각각 실행하며 결과를 해당 파일의 이름으로 로그 파일에 저장합니다.

이제 .bat 스크립트를 작성하여 SQL*Plus에 접속하고 run_all_sql.sql 스크립트를 실행합니다.
...
위의 .bat 스크립트를 실행하면 SQL*Plus에서 run_all_sql.sql 스크립트가 실행되고, 지정된 디렉토리의 모든 .sql 파일이 실행됩니다. 각각의 .sql 파일 실행 결과는 해당 파일의 이름으로 된 .log 파일에 저장됩니다.
참고로 이 스크립트에서는 dirlist 함수를 사용하여 디렉토리의 파일 목록을 가져옵니다. dirlist 함수는 Oracle에서 기본적으로 제공되지 않으므로, 이 함수를 사용하려면 먼저 사용자 정의 함수로 작성하여 데이터베이스에 추가해야 합니다.
